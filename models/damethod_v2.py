...
def apply_damethod_v2(Xs, Xt, ys=None, yt=None, 
                    dataset_name=None, src_name=None, 
                    tgt_name=None, batch_size=32, train_pct=0.8,
                    epochs=50, device='cuda:0', weighted_testing=True):
    ...
    dimensions_dict = {'mnist_a10': 28, 'mnist_m_a10': 28, 'mnist_lesser_a10': 28, 'usps_a10': 16, 
                        ...
                        'mnist_to_mnist_m_a10': 28, 'mnist_to_svhn_a10': 32, 
                        'mnist_to_usps_a10': 28, 'svhn_to_mnist_a10': 32, 
                        'synthdigits_to_svhn_a10': 32, 'usps_to_mnist_a10': 28}
    
    h_w = dimensions_dict[dataset_name]

    tensorboard_dir = '/home/tanwl/LocIT/damethod_v2_tensorboard_amended/' + dataset_name + '/' + src_name + '_to_' + tgt_name
    ...
    optimizer_finetune = OptimWithScheduler(
        optim.SGD(feature_extractor.parameters(), lr=0.001 / 10.0, weight_decay=0.0005, momentum=0.9, nesterov=True),
        scheduler)
    ...
    optimizer_discriminator_adv = OptimWithScheduler(
        optim.SGD(discriminator_adv.parameters(), lr=0.001, weight_decay=0.0005, momentum=0.9, nesterov=True),
        scheduler)

    optimizer_discriminator_non_adv = OptimWithScheduler(
        optim.SGD(discriminator_non_adv.parameters(), lr=0.001, weight_decay=0.0005, momentum=0.9, nesterov=True),
        scheduler)

    classifier_train_loss_lst = []
    classifier_val_loss_lst = []
    disc_adv_loss_lst = []
    disc_non_adv_loss_lst = []
    total_loss_lst = []

    classifier_train_losses = AverageMeter()
    classifier_val_losses = AverageMeter()
    disc_adv_losses = AverageMeter()
    disc_non_adv_losses = AverageMeter()
    ...
    for epoch in range(1, epochs+1):
        ...
        for step, ((source_samples, source_labels), (target_samples, target_labels)) in tqdm(enumerate(zip(train_dataloader, test_dataloader)), position=0, leave=True):
            ...
            classifier_train_loss = criterion(source_probs, source_labels_gpu.long())
            classifier_train_loss = torch.mean(classifier_train_loss, dim=0, keepdim=True)   
            ...
            classifier_train_losses.update(classifier_train_loss)
            disc_adv_losses.update(adv_loss)
            disc_non_adv_losses.update(non_adv_loss)

            with OptimizerManager(
                    [optimizer_finetune, optimizer_classifier, optimizer_discriminator_adv, optimizer_discriminator_non_adv]):
                total_loss = classifier_train_loss + adv_loss + non_adv_loss
                total_losses.update(total_loss)
                total_loss.backward()

            global_step = global_step + 1
        
        writer.add_scalar('classifier_train_loss', classifier_train_losses.avg, epoch)
        writer.add_scalar('disc_adv_loss', disc_adv_losses.avg, epoch)
        writer.add_scalar('disc_non_adv_loss', disc_non_adv_losses.avg, epoch)
        writer.add_scalar('total_loss', total_losses.avg, epoch)
        
        classifier_train_loss_lst.append(classifier_train_losses.avg.item())
        disc_adv_loss_lst.append(disc_adv_losses.avg.item())
        disc_non_adv_loss_lst.append(disc_non_adv_losses.avg.item())
        total_loss_lst.append(total_losses.avg.item())
        ...
        print('Training Loss: {}'.format(classifier_train_losses.avg.item()))

        # Calculating Validation Accuracy and Loss for this Epoch
        correct = 0
        total = len(val_data)
        feature_extractor.eval()
        classifier.eval()
        ...
        with torch.no_grad():
            for samples, labels in tqdm(val_dataloader, position=0, leave=True):
                ...
                classifier_val_loss = criterion(probs, labels_gpu.long())
                classifier_val_loss = torch.mean(classifier_val_loss, dim=0, keepdim=True)

                classifier_val_losses.update(classifier_val_loss)
        ...
        print('Validation Loss: {}'.format(classifier_val_losses.avg.item()))
        classifier_val_loss_lst.append(classifier_val_losses.avg.item())
        writer.add_scalar('classifier_val_loss', classifier_val_losses.avg, epoch)
        ...
        # Calculating Test Accuracy for this Epoch
        correct = 0
        total = len(test_data)
        feature_extractor.eval()
        classifier.eval()
        ...
        classifier_test_accuracy_lst.append(correct.item()/total)
        writer.add_scalar('classifier_test_accuracy', correct.item()/total, epoch)
    ...
    with open(tensorboard_dir + '/classifier_train_loss_lst.json', 'w') as f:
        json.dump(classifier_train_loss_lst, f)
    with open(tensorboard_dir + '/classifier_val_loss_lst.json', 'w') as f:
        json.dump(classifier_val_loss_lst, f)
    ...
    with open(tensorboard_dir + '/classifier_test_accuracy_lst.json', 'w') as f:
        json.dump(classifier_test_accuracy_lst, f)
    ...
    if weighted_testing == True:
        # Implement semi-supervised anomaly classification
        feature_extractor.eval()
        classifier.eval()
        discriminator_adv.eval()
        discriminator_non_adv.eval()
        ...
        with torch.no_grad():
            for samples, labels in test_dataloader_final:
                ...
                out = feature_extractor(samples_gpu)
                input_features, hidden_layer_features, _, logits, probs = classifier(out)
                domain_prob_discriminator_non_adv = discriminator_non_adv(hidden_layer_features)
                target_share_weight = get_target_share_weight(domain_prob_discriminator_non_adv, logits, 
                                                            domain_temperature=1.0, class_temperature=1.0)

        # each probs is tensor([0.6760, 0.3240]) -> classifier output
        # each label is tensor(0.) or tensor(1.) -> ground-truth normal or anomaly label
        # each target_share_weight is tensor([-0.4122]) -> target_share_weight

        hidden_layer_features_numpy = hidden_layer_features.detach().cpu().numpy() # (550, 256)
        labels_numpy = labels_gpu.detach().cpu().numpy() # (550,)
        target_share_weight_numpy = target_share_weight.detach().cpu().numpy().squeeze() # (550,)
        probs_numpy = probs.detach().cpu().numpy()[:, 1] # (550,)

        # Permute everything to be [known samples, unknown samples]
        # i.e. if less than threshold, the sample is "unknown" and needs to be classified by kNN
        # i.e. if more than threshold, the sample is "known" and can be classified by the learnt classifier
        threshold = (float(target_share_weight_numpy.min()) + float(target_share_weight_numpy.max())) / 2
        
        idx_unknown = target_share_weight_numpy < threshold
        idx_known = target_share_weight_numpy >= threshold

        unknown_count = np.count_nonzero(idx_unknown == True)
        known_count = np.count_nonzero(idx_known == True)

        logging.info('Known: ' + str(known_count) + ' | ' + 'Unknown: ' + str(unknown_count) + ' | ' + 'Weight Min: ' + str(round(float(target_share_weight_numpy.min()), 3)) + ' | ' + 'Weight Max: ' + str(round(float(target_share_weight_numpy.max()), 3)))
        
        weight_statistics = {}
        weight_statistics['max'] = round(float(target_share_weight_numpy.max()), 3)
        weight_statistics['min'] =  round(float(target_share_weight_numpy.min()), 3)
        weight_statistics['threshold'] = round(threshold, 3)
        weight_statistics['known'] = known_count
        weight_statistics['unknown'] = unknown_count

        with open(tensorboard_dir + '/weight_statistics.json', 'w') as f:
            json.dump(weight_statistics, f)

        if unknown_count > 0:
            #hidden_layer_features_numpy_permutated = np.concatenate((hidden_layer_features_numpy[idx_known], hidden_layer_features_numpy[idx_unknown]))
            labels_numpy_permutated = np.concatenate((labels_numpy[idx_known], labels_numpy[idx_unknown]))
            #probs_numpy_permutated = np.concatenate((probs_numpy[idx_known], probs_numpy[idx_unknown]))

            knno_scores = apply_kNNO(Xs=None, Xt=hidden_layer_features_numpy, ys=None, yt=labels_numpy)

            final_probs = np.concatenate((probs_numpy[idx_known], knno_scores[idx_unknown]))
        
        else:
            labels_numpy_permutated = labels_numpy
            final_probs = probs_numpy

        assert final_probs.shape == probs_numpy.shape, "Wrong semi-supervised probs shape!"

        # roc_auc_score needs (550,) and (550,) as inputs
        auc = roc_auc_score(y_true=labels_numpy_permutated, y_score=final_probs)
